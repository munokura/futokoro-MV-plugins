...
//=============================================================================
/*:
@plugindesc v1.1.0 Plugin to ignore errors in scripts executed by events
@author Futokoro
...
Overview
-----------------------------------------------------------------------------
This plugin continues processing events even if an error occurs in a script command executed within the event.

Script errors are displayed on the console screen.
...
Notes for Use
-----------------------------------------------------------------------------
Normally, a script error will cause the game to stop.

This plugin logs errors, but allows the game to continue.

However, ignoring script errors and proceeding with the event may cause the event to behave strangely.
Depending on how the event is configured, this may result in an infinite loop or automatic events that do not stop.
...
v1.1.0 - 2018/08/13: Supports FTKR_ConvertEscapeCharactersInScript.js

v1.0.0 - 2017/05/03: First version created

-----------------------------------------------------------------------------

@param Display Error Log
@desc Whether script errors should be displayed in the console: 1 - yes, 0 - no
@default 1
*/
...
@param Display Error Log
@desc スクリプトのエラーをコンソールに表示するか 1 - 表示する, 0 - 表示しない
@default 1
*/
