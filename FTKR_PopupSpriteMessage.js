...
/*~struct~popup:
@param fontFace
@desc Enter the font to use. If left blank, the MV default font will be used.
...
@param color
@desc Specifies the color of the string. The meaning of each list number value is as follows: 1: Red, 2: Green, 3: Blue, 4: Gray (specify in the range of 0 to 255)
@default ["0","0","0","0"]
@type number[]

@param italic
@desc whether to display in italics
@default false
@type boolean
@on valid
@off invalid
...
@param outlineColor
@desc Specify the color number to display the text with a border (0 to 31). Leave blank to display no border.
@default 15

@param popupHeight
@desc Pop-up bounce height
@default 40
@type number
@min 0

@param offsetWait
@desc The time interval when displaying characters one by one. If 0, they are displayed simultaneously.
@default 0
@type number
@min 0
...
@param Popup Message Status
@desc ポップアップ表示する際の設定 複数のパターンを設定し、プラグインコマンドで呼び出し可能
@default ["{\"fontFace\":\"\",\"fontSize\":\"28\",\"color\":\"[\\\"0\\\",\\\"0\\\",\\\"0\\\",\\\"0\\\"]\",\"italic\":\"false\",\"outlineColor\":\"15\",\"popupHeight\":\"40\",\"duration\":\"90\"}"]
@type struct<popup>[]
...
/*~struct~popup:ja
@param fontFace
@desc 使用するフォントを記述 空欄の場合はMVデフォルトフォントを使用
...
@param color
@desc 文字列の色を指定、各リスト番号の値の意味は以下 1:赤, 2:緑 ,3:青 ,4:グレー (0~255の範囲で指定)
@default ["0","0","0","0"]
@type number[]
...
@param outlineColor
@desc 文字を縁取り表示する場合にカラー番号を指定(0~31) 空欄は縁取りなし
@default 15
...
@param offsetWait
@desc 文字を一文字ずつ表示させる場合の時間間隔 0 の場合は、同時に表示
@default 0
@type number
@min 0
